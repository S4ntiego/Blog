...
To build the project, we will be using the newest Next.js version available as of May 19, 2023 - 13.4 and utilise the new App Router that is now production-ready. To style the blog, we will use the Tailwind CSS library.
...
2.  Launch your IDE and open the previously created folder. If you're using Visual Studio Code, in the top control panel go to File -> Open Folder, find your folder, and click "Open."
3.  Open the terminal. In the top control panel of Visual Studio Code, go to Terminal -> New Terminal. You can also create a new terminal using the following shortcut in VSCode: `` Ctrl Shift ` `` on Windows or `` ⌃ ⇧ ` `` on Mac.
...
On installation, you will see prompts regarding TypeScript, ESLint, Tailwind CSS, /src directory, App Router and default import alias, I recommend the following configuration:
...
By default, the website should run on port 3000, but if the given port is already in use, the website will try to run on another port instead, which will be visible in your terminal message like this:
...
To visit the website, open your browser (I highly recommend using Google Chrome, due to the amazing Developer Tools) and type the above url in the address bar.
...
[Contentlayer](https://www.contentlayer.dev/) will help us to validate and transform our content written in mdx into type-safe JSON data, that we will be able to easily import into our application (we are using Contentlayer to gain the advantage of working with MDX files - Markdown with embedded JSX).
...
To hook Contentlayer into the `next dev` and `next build` processes, we need to wrap the Next.js configuration using the withContentlayer method in next.config file.

1. To be able to use the `import` syntax instead of `require`, change the extension of your next.config from `.js` to `.mjs`
2. Change the code as presented below:
...
In this project, we will build a very simple blog. For that, we will need to define the schema of our posts, so let's create a `contentlayer.config.ts` file in the root directory of our project, and add the following content that will define our `Post`:
...
- In the code above, we are importing `allPosts` from `contentlayer/generated`, this is where our content lives. We are also importing our date helper function and exporting metadata, which is an abstract of our website's content and is used to attach a title (in this case) to the site. Title is what users see when they click to enter our website from search results, it is also one of the main elements Google uses to understand the content of our page.

- Then, we are importing all of our posts by running a simple function `const posts = allPosts.filter((post) => post.published)`. We are filtering our content to include only the posts that are ready to be published (defaults to true, unless we would like to state otherwise in each post).

- At the end, we are adding structure to our post by using html elements styled using the Tailwind CSS library. We are conditionally rendering our posts by using the `ternary operator`, if posts exist, then we want to map over all of them and render each post's date, description and title with the link to view it's body, which we will configure in a second, if posts don't exist, we return a simple paragraph that informs the user that no posts are published yet:
  `{posts?.length ? () : ()}`
...
You can create your own schemas and add field such as Authors, Categories, sort posts in descending order, add your own jsx components and embed them directly in your content.
